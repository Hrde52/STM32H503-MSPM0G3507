...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == USART2)
    {
        ...
        if (rxIndex >= sizeof(SensorProtocol))
        {
            ...
            global_pkt = pkt; // 更新全局变量

            uint8_t sizeSP = sizeof(SensorProtocol);
            total_len = sizeSP - 3 /*cmd+l1*/ + pkt->length; // 动态计算完整包长度（固定字段 + DATA1）
            if (total_len >= MAX_RX_LEN)
            {
                rxIndex = 0;
                total_len = 0;
                memset(rxDBuffPDA, 0, sizeof(rxDBuffPDA)); // 清空缓存数组
            ...
            if (rxIndex >= total_len - 1) // 当前包接收完毕
            {
                ProcessPacket(pkt);
                // 接收数据处理完毕，继续下一个数据包的接收
                rxIndex = 0;
                total_len = 0;
                memset(rxDBuffPDA, 0, sizeof(rxDBuffPDA)); // 清空缓存数组
                RS485_PDA_RX_ENABLE();
...
void ProcessElevatorData(void)
{
    ...
    if (isFixBytes && rx_data_Elevator[0] == rxDataEleHeander)
    {
        ...
        if (checkXor == rx_data_Elevator[4])
        {
            ...
            主控下发的信号
            */
            masterElevator_LevelingSignal = (rx_data_Elevator[1] >> 0) & 0x01; // 平层信号
            ...
            /*
                ...
                byte1
                    ...
                    5 夹手
                    4 无
                    3 -0 传感器工作状态 ：00工作正常，01工作异常，02调试模式
                byte5: 故障码
            */
            tx_data_Door[0] = rx_data_Elevator[0];

            // 方法2：位域操作
            ByteBits *byte_bits = (ByteBits *)&tx_data_Door[1]; // 指向 packet[1]
            ...
            byte_bits->bits.bit4 = 0; // bit4=无
            // byte_bits->bits.bit5 = fingerDetected_flag;              // bit5=夹手状态检出
            // byte_bits->bits.bit6 = regionalObjectDetected_flag_ND06; // bit6=区域物体检出
            byte_bits->bits.bit7 = IO_dts6012 || IO_ND06; // bit7=IO动作标志
            // byte_bits->bits.bit7 = 1; // bit7=IO动作标志
    ...
    else
    {
        ...
        if (crcValue == rx_data_Elevator[rxSize - 1]) // 正确接收到数据
        {
            // rs485ElevatorRxCpltFlag = 1;

            // memcpy(RxDataElevator, rx_data_Elevator, sizeof(rx_data_Elevator));

            if (isSixBytes) // 接获取主控信号
            {
                if ((rx_data_Elevator[1] & 0X01))
                    CSpara.openDoorCmd = 1; // 获取开门指令
                else
                    CSpara.openDoorCmd = 0;

                if ((rx_data_Elevator[1] & 0X02))
                    CSpara.closeDoorCmd = 1; // 获取关门指令
                else
                    CSpara.closeDoorCmd = 0;
            }
            else // 获取到位信号、平层信号
            {
                if ((rx_data_Elevator[1] & 0X04)) // HCB板发出的平层信号,低无效
                    CSpara.LevelingSignal = 0;
                else
                    CSpara.LevelingSignal = 1;

                if ((rx_data_Elevator[1] & 0X10)) // 开门到位信号,低有效
                {
                    if (PARA_TABLE_USE.data.sensorAddr != 0x51)
                    {
                        CSpara.OLS = 0; // 大石一楼1，二楼0，楼宇0
                    }
                    else
                    {
                        CSpara.OLS = 1;
            ...
            isSixBytes = !isSixBytes; // 交替接收6字节主控，与4字节的HCB板信息
        }
    }

    headerIsFoundFlag = 0;
    RxElevatorCnt = 0;
    memset(rx_data_Elevator, 0, sizeof(rx_data_Elevator)); // 清空缓存数组
    return;
}
...
void ProcessGHPTooling(uint8_t *data)
{
		...
		if (res1 == 0)
    {
		...
		HAL_UART_Transmit(&huart2, resultUp, 20, 100);
